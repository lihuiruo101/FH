...
class AccountHeaders:
    ...
    def get_headers_static(iq: str, data: str):
        return {                                                  "accept-enconding": "gzip",
            "connection": "keep-alive",
            "content-type": "application/json",                   "nyanko-signature": core.NyankoSignature(iq, data).generate_signature(),
            "nyanko-timestamp": str(int(time.time())),            "nyanko-signature-version": "1",
            "nyanko-signature-algorithm": "HMACSHA256",                                                                 "user-agent": "Dalvik/2.1.0 (Linux; U; Android 9; SM-G955F Build/N2G48B)",
        }                                             .../site-packages/bcps $ cat core/server/managed_item.py
from __future__ import annotations                    
"""ManagedItem class for bcps."""                     
from enum import Enum
from typing import Any
import uuid
import time                                           from bcps import core

                                                      class DetailType(Enum):                                   """Enum for the different types of details."""
...
class ManagedItem:
    ...
    def __init__(                                             self,
        amount: int,                                          detail_type: DetailType,
        managed_item_type: ManagedItemType,
        detail_code: str = "",
        detail_created_at: int = 0,
    ):                                                        self.amount = amount
        self.detail_type = detail_type
        self.managed_item_type = managed_item_type            if not detail_code:
            detail_code = str(uuid.uuid4())                   self.detail_code = detail_code
        if not detail_created_at:                                 detail_created_at = int(time.time())
    ...
    def from_change(change: int, managed_item_type: ManagedItemType) -> ManagedItem:
        ...
        return managed_item                           
    def to_dict(self) -> dict[str, Any]:                      """Convert the managed item to a dictionary."""

        data = {
            "amount": self.amount,                                "detailCode": self.detail_code,
            "detailCreatedAt": self.detail_created_at,            "detailType": self.detail_type.value,
    ...
    def from_short_form(short_form: str) -> ManagedItem:
        ...
        try:
            amount = int(values[0])                           except (IndexError, ValueError):
            amount = 0

        try:
            detail_created_at = int(values[1])                except (IndexError, ValueError):
            detail_created_at = 0
        ...
        return ManagedItem(                                       amount,
            DetailType(detail_type),                              ManagedItemType(managed_item_type),
    ...
    def __init__(                                             self,                                                 save_file: core.SaveFile,
        ...
        self.identifier = "managed_items"                                                                       def set_managed_items(self, managed_items: list[ManagedItem]):
        self.save_file.remove_strings(self.identifier)        for managed_item in managed_items:
            string = managed_item.to_short_form()
            self.save_file.store_string(self.identifier, string, overwrite=False)                           
    def get_managed_items(self) -> list[ManagedItem]:         managed_items: list[ManagedItem] = []                 managed_items_str = self.save_file.get_strings(self.identifier)
        for managed_item_str in managed_items_str:                managed_item = ManagedItem.from_short_form(managed_item_str)
            if managed_item.amount == 0:
                continue
            managed_items.append(managed_item)                return managed_items
                                                          def add_managed_item(self, managed_item: ManagedItem):
        if managed_item.amount == 0:
            return
        managed_items = self.get_managed_items()              managed_items.append(managed_item)
    ...
    def create(
        ...
        return BackupMetaData.create_static(                      self.save_file.inquiry_code,
            self.save_file.officer_pass.play_time,
            self.save_file.calculate_user_rank(),
            self.get_managed_items(),                             save_key,
            add_managed_items,
        )
                                                          @staticmethod                                         def create_static(
        iq: str,
        playtime: int,                                        userrank: int,
        items: list[ManagedItem],                             save_key: str | None = None,
        ...
        managed_items_str = (
            ...
            "rank": userrank,
            "receiptLogIds": [],
            "signature_v1": core.NyankoSignature(                     iq, managed_items_str
            ).generate_signature_v1(),
        }
        if save_key is not None:                                  backup_metadata["saveKey"] = save_key
        return (
