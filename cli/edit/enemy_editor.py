...
class EnemyEditor:
    ...
    def select(self, current_enemies: list[core.Enemy] | None):
        ...
        if option_id is None:
            return current_enemies                            option_id -= 1
        ...
        if current_enemies:
            ...
            if mode_id is None:
                ...
                mode = SelectMode.OR                          else:
            mode = SelectMode.OR                      
        if mode == SelectMode.AND:                                return [enemy for enemy in new_enemies if enemy in current_enemies]
        if mode == SelectMode.OR:                                 return list(set(current_enemies + new_enemies))
        if mode == SelectMode.REPLACE:                            return new_enemies
        return new_enemies
                                                          def get_all_enemies(self) -> list[core.Enemy]:
        enemies: list[core.Enemy] = []
        for i in range(len(self.save_file.enemy_guide)):
            enemies.append(core.Enemy(i))                     return enemies
                                                          def select_id(self) -> list[core.Enemy] | None:
        enemy_ids = dialog_creator.RangeInput(                    len(self.save_file.enemy_guide) - 1
    ...
    def select_name(self) -> list[core.Enemy] | None:
        ...
        for enemy_name in enemy_names:                            if enemy_name is None:
                return None

            new_enemy_names.append(enemy_name)        
        enemy_option_ids, _ = dialog_creator.ChoiceInput.from_reduced(                                                  new_enemy_names, dialog="select_enemies", single_choice=False
        ).multiple_choice()
        if enemy_option_ids is None:                              return None
    ...
    def get_enemies_by_name(self, name: str) -> list[core.Enemy]:
        enemies: list[core.Enemy] = []                        for enemy in self.get_all_enemies():
            enemy_name = enemy.get_name(self.save_file)
            if enemy_name is None:                                    continue
