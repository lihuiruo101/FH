...
class CatEditor:
    ...
    def upgrade_talents_remove_talents_cats_run(
        ...
        elif choice == 1:                                         cat_editor.remove_talents_cats(current_cats)

    @staticmethod                                         def unlock_cat_guide_remove_guide_run(                    save_file: core.SaveFile,                             current_cats: list[core.Cat] | None = None,
        cat_editor: CatEditor | None = None,              ):                                                        if cat_editor is None or current_cats is None:            cat_editor, current_cats = CatEditor.from_save_file(save_file)
        if cat_editor is None:
            return
        choice = dialog_creator.ChoiceInput(
    ...
    def run_edit_cats(
        ...
        cat_editor = CatEditor(save_file)
        cat_editor.print_selected_cats(cats)                  options: list[str] = [
            "select_cats_again",                                  "unlock_remove_cats",
            "upgrade_cats",
        ...
        if option_id is None:                                     return False, cats
        option_id -= 1
        if option_id == 0:
            cats_ = cat_editor.select(cats)
            if cats_ is None:
                return False, cats                                cats = cats_
        elif option_id == 1:
            cat_editor.unlock_remove_cats_run(save_file, cats, cat_editor)                                          elif option_id == 2:                                      cat_editor.upgrade_cats(cats)                     elif option_id == 3:                                      cat_editor.true_form_remove_form_cats_run(save_file, cats, cat_editor)                                  elif option_id == 4:
            color.ColoredText.localize("force_true_form_cats_warning")
            cat_editor.true_form_cats(cats, force=True)                                                             elif option_id == 5:
            cat_editor.fourth_form_remove_form_cats_run(save_file, cats, cat_editor)
        elif option_id == 6:
            color.ColoredText.localize("force_fourth_form_cats_warning")
            cat_editor.fourth_form_cats(cats, force=True)
        elif option_id == 7:                                      cat_editor.upgrade_talents_remove_talents_cats_run(
                save_file, cats, cat_editor
            )                                                 elif option_id == 8:
            cat_editor.unlock_cat_guide_remove_guide_run(save_file, cats, cat_editor)
        CatEditor.set_rank_up_sale(save_file)
        if option_id == 9:
            return True, cats                                 return False, cats                                                                                      @staticmethod
