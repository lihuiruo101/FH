...
from bcps.cli import color, dialog_creator            from typing import Union
...
def unclear_rest(                                         chapters: ChaptersType,
    stages: list[int],
    stars: int,
    id: int,
    type: int | None = None,
...
def get_total_stars(chapters: ChaptersType, id: int, type: int | None = None):
    if isinstance(chapters, core.EventChapters):
        if type is None:                                          raise ValueError("Type must be specified for EventChapters!")                                           total_stars = chapters.get_total_stars(type, id)
    else:                                                     total_stars = chapters.get_total_stars(id)
...
def get_total_stages(
    ...
    else:                                                     total_stars = chapters.get_total_stages(id, star)

    return total_stars                                

def edit_chapters(
    save_file: core.SaveFile,
    chapters: ChaptersType,                               letter_code: str,
    type: int | None = None,
):
    map_names = core.MapNames(save_file, letter_code)
    names = map_names.map_names                       
    choice = dialog_creator.ChoiceInput.from_reduced(
        ["clear_stages", "unclear_stages"], dialog="clear_unclear_q", single_choice=True                        ).single_choice()
    if choice is None:
        return                                        
    choice -= 1
                                                          clear = choice == 0
    ...
    else:                                                     modify_clear_amounts = False
                                                          if modify_clear_amounts is None:
        return

    clear_amount = 1
    clear_amount_type = -1                                if modify_clear_amounts:
        options = ["clear_amount_chapter", "clear_amount_all"]
        if clear_type_choice == 1:
            options.append("clear_amount_stages")             clear_amount_type = dialog_creator.ChoiceInput.from_reduced(
            options, dialog="select_clear_amount_type", single_choice=True                                          ).single_choice()
        if clear_amount_type is None:
            return
        clear_amount_type -= 1
        if clear_amount_type == 1:                                clear_amount = core.EventChapters.ask_clear_amount()
    ...
    for id in map_choices:
        ...
        if stars_type_choice:
            ...
            if stars is None:
                return
        if clear_type_choice:                                     stages = core.EventChapters.ask_stages(map_names, id)
            if stages is None:
                return
else:
